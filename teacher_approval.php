...
$query_access  = "INSERT INTO `user_activity`(`user_id`, `user_role`, `accessed_page`, `access_date`, `access_time`) VALUES ('$adminid','$role','teacher approval',now(),now())";
$result_access = mysqli_query($conn,$query_access);

$teacher_approval_query  = "SELECT * FROM `teacher_login` WHERE `approve_status`='0'";
$teacher_approval_result = mysqli_query($conn,$teacher_approval_query);
$teacher_approval_array  = mysqli_fetch_all($teacher_approval_result);



$temp_array = [];

foreach($teacher_approval_array as $value => $values)
{
    $array['serial_no']       = $values['0'];
    $array['teacher_name']    = $values['1'];
    $array['teacher_dept']    = $values['2'];
    $array['teacher_type']    = $values['6'];
    $array['teacher_email']   = $values['4'];
    $array['teacher_phoneno'] = $values['3'];
    $array['teacher_approve'] = $values['9'];
            ...
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="teacher_approval.php" class="dropdown-item active">Teacher</a>
				...
				<table class="table table-bordered table-hovered table-striped" id="productTable">
                    ...
                    if(!empty($temp_array)) {
                        ...
                        foreach($temp_array as $val){ ?>
								...
								<td> <?= $val['teacher_dept'] ?> </td>
								<td> <?php if($val['teacher_type'] == 2){echo 'HOD';}else{ echo 'AST.TEACHER'; } ?> </td>
                                <td> <?= $val['teacher_email']?> </td>
								<td> <?= $val['teacher_phoneno'] ?> </td>
...
<script>
    ...
    function update_status(userid){
        ...
        var user_id = userid;
        let url = "http://localhost/php_code/teacher_approve_update.php?user_id="+user_id;

...
$query_access  = "INSERT INTO `user_activity`(`user_id`, `user_role`, `accessed_page`, `access_date`, `access_time`) VALUES ('$adminid','$role','download e learning',now(),now())";
$result_access = mysqli_query($conn,$query_access);

$query_file_upload  = "SELECT * FROM `file_upload`";
$result_file_upload = mysqli_query($conn,$query_file_upload);
$file_upload_array  = mysqli_fetch_all($result_file_upload); 



$temp_array = [];

foreach($file_upload_array as $value => $values)
{
    $array['serial_no']  = $values['0'];
    $array['title']      = $values['1'];
    $array['subject']    = $values['2'];
    $array['file_name']  = $values['3'];
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="teacher_registration.php" class="dropdown-item active">Teacher</a>
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            ...
                            <a href="download_elearning.php" class="dropdown-item active">Download Books</a>
                        </div>
                    </div>
			...
			<div class="col-xl-12 col-lg-12 col-md-12 col-12 m-auto">
				<table class="table table-bordered table-hovered table-striped" id="productTable">
					<thead>
						...
						<th> Subject </th>
						<th> Title </th>
						<th> <?php if($role==2||$role==3){ ?>Download/Remove<?php }else{ ?>Download<?php } ?> </th>
					</thead>
                    ...
                    if(!empty($temp_array)) {
                        ...
                        foreach($temp_array as $val){ ?>
                                ...
                                <td> <?= $val['title'] ?> </td>
								<td> <?= $val['subject'] ?> </td>
								<td><button class="btn btn-success rounded-pill m-2"><a href="uploads/<?= $val['file_name']?>" target="_blank" style="color:white">Download</a></button>
                            <?php if($role==2||$role==3){ ?>

                                <button class="btn btn-danger rounded-pill m-2" onclick="update_status(<?php echo $val['serial_no'] ?>)">Remove</button>
...
function update_status(userid){
    ...
    //alert(userid);
var user_id = userid;
let url = "http://localhost/php_code/delete_file.php?file_id="+user_id;
